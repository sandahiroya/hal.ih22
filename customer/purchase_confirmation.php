...
if(isset($_SESSION['old'])){
  ...
  //var_dump($address_row['customer_address']);
...
for ($i=0; $i < count($_SESSION['product']); $i++) { 
  $product = mysqli_query($link,"SELECT product_id,category_id,brand_id,product_name,product_price,exhibit_price FROM product WHERE product_id =". $_SESSION['product'][$i]);
...
$sql = "SELECT COUNT(*) FROM stamp WHERE customer_id = ".$_SESSION['customer']."";

$result = mysqli_query($link,$sql);
if(!$result){
  echo '予期せぬエラーが発生しました。しばらくたってから再度お試しください。(エラーコード：103)';
  // require_once './tpl/error.php';
  exit;
}
...
if(isset($_POST['purchase'])){
  ...
  $sql_b = "INSERT INTO product_order (customer_id,order_id,order_day,pay_way,delivery_day,delivery_price,tax,sum_price) 
                              VALUES (".$_SESSION['customer'].",".$order_num2.",".$sold_data.",'".$_SESSION['payment']."',".$delivery_day.",".$delivery_price.",100,".$money.")";

  $result = mysqli_query($link,$sql_a);

  $result2 = mysqli_query($link,$sql_b);

  for($cnt = 0; $cnt < count($cart_list); $cnt++){
    $sql = "INSERT INTO order_detail (product_id,order_id,unit_price) VALUE(".$cart_list[$cnt]['product_id'].",".$order_num2.",".$cart_list[$cnt]['product_price'].")";
    var_dump($sql);
    $result = mysqli_query($link,$sql);
    if(!$result){
      echo  "予期せぬエラーが発生しました。しばらく経ってから再度お試しください。(222)";
      // require_once './tpl/error.php';
      exit;
    }
  }
