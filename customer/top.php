...
if(isset($_SESSION['b_id'])){
  ...
  if(isset($_POST['sea2']) && $_POST['sea'] != "" || isset($_SESSION['sea']) && $_SESSION['sea'] != ""  && !isset($_POST['sea2']) ){
    ...
    $page = mysqli_query($link,"SELECT * FROM product WHERE  sold_day IS  NULL AND product_delet IS  NULL ");
    $page_num = mysqli_num_rows($page);//現在のテーブルのデータの件数
    ...
    $start_no = ($now - 1) * $display;
    $product = mysqli_query($link,"SELECT * FROM product WHERE  sold_day IS  NULL AND product_delet IS  NULL LIMIT " . $start_no .",9" );
